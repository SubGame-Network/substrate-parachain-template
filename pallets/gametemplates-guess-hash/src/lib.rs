				...
				for game_id in game_id_list {
					...
					for (k, v) in bet_list.iter().enumerate() {
						...
						if v.game_mode == ResultGameMode.unwrap() {	
							...
							T::Chips::unreserve(&v.user, v.amount);
							// Owner issues rewards to punters
							T::Chips::repatriate_reserved(&owner, &v.user, v.amount);
						...
						// loser
							...
							T::Chips::repatriate_reserved(&v.user, &owner, v.amount);
					...
					T::Chips::unreserve(&owner, owner_pool);
